...
class GeMapHead(DETRHead):
    ...
    def forward(self, mlvl_feats, lidar_feat, img_metas, prev_bev=None,  only_bev=False, is_train=False):
        """Forward function.
        Args:
        ...
        elif self.query_embed_type == 'geo_ca':
    ...
    def loss_single(self,
        ...
        loss_geo_2 = torch.zeros((1,), device = pts_preds.device)
        if self.use_geo_loss:
            loss_geo_2 = self.loss_geo(pts_preds, pts_targets, labels)
            # # intra
            # intra_loss = 0
            # ft_preds = pts_preds[labels < 3]
            # ft_targets = normalized_pts_targets[labels < 3]
            ...
            # intra_loss += self.loss_geo(length_preds[isnotnan], length_targets[isnotnan])
            # isnotnan = torch.isfinite(dot_targets)
            # intra_loss += self.loss_geo(dot_preds[isnotnan], dot_targets[isnotnan])
            # isnotnan = torch.isfinite(cross_targets)
            # intra_loss += self.loss_geo(cross_preds[isnotnan], cross_targets[isnotnan])

            # # inter
            # inter_loss = 0
            # re_preds = pts_preds.view(-1, self.num_vec, self.num_pts_per_vec, 2)
            # re_targets = normalized_pts_targets.view(-1, self.num_vec, self.num_pts_per_vec, 2)
            ...
            #     ft_preds = re_preds[idx][labels_list[idx] < 3]
            #     ft_targets = re_targets[idx][labels_list[idx] < 3]
            #     ft_offset_preds = offset_re_preds[idx][labels_list[idx] < 3]
            #     ft_offset_targets = offset_re_targets[idx][labels_list[idx] < 3]
            ...
            #     isnotnan = torch.isfinite(length_targets)
            #     inter_loss += self.loss_geo(length_preds[isnotnan], length_targets[isnotnan])
            #     isnotnan = torch.isfinite(dot_targets)
            #     inter_loss += self.loss_geo(dot_preds[isnotnan], dot_targets[isnotnan])
            #     isnotnan = torch.isfinite(cross_targets)
            #     inter_loss += self.loss_geo(cross_preds[isnotnan], cross_targets[isnotnan])
            # if self.inter_loss_weight is not None or self.intra_loss_weight is not None:
            #     loss_geo_2 = 0
            #     if self.inter_loss_weight is not None:
            #         loss_geo_2 += self.inter_loss_weight * inter_loss
            #     else:
            #         loss_geo_2 += inter_loss
            #     if self.intra_loss_weight is not None:
            #         loss_geo_2 += self.intra_loss_weight * intra_loss
            #     else:
            #         loss_geo_2 += intra_loss
            # else:
    ...
    def loss(self,
        ...
        init_reference = preds_dicts['init_reference']
